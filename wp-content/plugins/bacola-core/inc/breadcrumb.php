...
if ( ! function_exists( 'bacola_breadcrubms' ) ) {
    function bacola_breadcrubms() {
        ...
        if ( $on_front == "page" && is_home() ) {
            $blog_title = isset( $blog_page ) ? get_the_title( $blog_page ) : esc_html__( 'Our Blog', 'bacola' );
            $out .= sprintf( $link, home_url(), esc_html__( 'Home', 'bacola' ) ) . $separator . sprintf( $current, $blog_title );
        } else {
            $out .= sprintf( $link, home_url(), esc_html__( 'Home', 'bacola' ) );
        }
        ...
        if ( is_singular() ) {
            ...
            if ( is_post_type_archive() ) {
                ...
                $out .= $separator . esc_html__( 'Category : ', 'bacola' ) . sprintf( $current, $wp_query->queried_object->name );
            } else if ( is_tag() ) {
                $out .= $separator . esc_html__( 'Tag : ', 'bacola' ) . sprintf( $current, $wp_query->queried_object->name );
            } else if ( is_date() ) {
                $out .= $separator;
                if ( is_day() ) {
                    global $wp_locale;
                ...
                $out .= $separator . sprintf( $current, esc_html__( 'Error 404', 'bacola' ) );
            } else if ( is_search() ) {
                $out .= $separator . sprintf( $current, esc_html__( 'Search', 'bacola' ) );
            }
